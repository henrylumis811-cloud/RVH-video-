...
## Legendary Step 99 — Signal Curation & Immersive Playback
- Garage Intelligence is now deliberately curated to three high-value surfaces: live intelligence, one command target, and one compact next-up queue; repetitive command dashboards were removed from the visible intelligence flow.
- Home rails are deduplicated so the same media item is not repeatedly surfaced across the curated Continue Watching / Recently Added area.
- Home Continue Watching and Recently Added now route by the media's actual category: Shorts open directly in Shorts, music videos open in the Music Video player, and movies open in the Movie player.
- Full-screen movie and music-video playback keeps the screen awake and hides Android system bars (including the clock and battery) for a clean cinematic surface; system chrome returns when leaving the player.


## Legendary Step 100 — Immersive Gesture Control
- Coordinated brightness/volume swipe handling so the left-side brightness gesture is not swallowed by the volume detector.
- Brightness uses the current window brightness when available and falls back to the device brightness setting, then applies a smooth per-gesture adjustment.
- Center double-tap toggles play/pause; left/right edge double-tap seeking remains -10s/+10s.
- Watching surfaces remain cinematic and car-background-free.
